...
createSubscription = function (customer, plan, coupon) {
	return new Promise(function (resolve, reject) {
		...
		if (coupon.InitialFee) {
			createCharge(customer, coupon.InitialFee).then(complete).catch(reject);
		}
		else {
			complete();
		}
...
// Creates a new stripe charge for a stripe customer
createCharge = function (customer, amount, description) {
	return new Promise(function (resolve, reject) {
...
var Stripe = {
	...
	//
	// Creates a one time charge for a specific profile
	//
	createCharge: function (profileID, opts) {
		return new Promise(function (resolve, reject) {

			var existingMethod = !!opts.PaymentMethodID;

			(existingMethod ? getCard(opts.PaymentMethodID) : createCard(profileID, opts)).then(function (card) {

				//
				// Check if customer record already exists for current profile
				//
				return getCustomer(profileID).then(function (existingCustomer) {

					var chargeDescription = 'Artwork Purchase';

					//
					// Create a new subscription for existing customer
					//
					if (existingMethod && existingCustomer) {
						return createCharge(existingCustomer, opts.amount, chargeDescription);
					}
						...
						return profileService.get.base(profileID).then(function (profile) {
							//
							// Create a new customer and subscription
							//
							var customerDescription = profile.Name + ' [' + profile.ID + ']';
							return createCustomer(profileID, card.Token, customerDescription).then(function (customer) {
								return db('PaymentProviders_Stripe_CustomerCards').where('ID', card.ID).update({ CustomerID: customer.ID }).then(function () {
									return createCharge(customer, opts.amount, chargeDescription);
								});
							})
						});
