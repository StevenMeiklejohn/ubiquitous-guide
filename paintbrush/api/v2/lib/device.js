...
var filterDeviceBrowser = function(devices, agent) {
	...
	devices.some(function(d) {
		if (d.BrowserName === agent.browser && d.BrowserVersion === agent.browserVersion) {
			device = d;
			device.ClientID = agent.clientID;
			return true;
		}
	});
	return device;
...
var Device = {
	...
	parseAgent: function(req) {
		return new Promise(function (resolve) {
			var promiseQueue = [], userID, clientID;
			...
			//console.log(JSON.parse(JSON.stringify(userAgent)));
			...
			var appNames = ['ActivCanvas', 'Wishbone Interactive'];
			appNames.some(function (name) {
				var idx = userAgentString.indexOf(name);
				if (idx > -1) {
					browser = name;
					browserVersion = userAgentString.slice(idx + name.length).split(/\/| /g)[1];
					return true;
				}
			});
