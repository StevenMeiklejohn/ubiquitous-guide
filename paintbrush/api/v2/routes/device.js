			...
			.then(function (device) {
				...
				if (!device) {
					...
					return db('Devices').where('ID', id).update({
						...
						OneSignalUserID: req.body.OneSignalUserID || device.OneSignalUserID
					})
					.then(function() {

						// Update DeviceBrowser record with push tokens
						Device.current(req).then(function(device) {
							return db('DeviceBrowsers').where('ID', device.BrowserID).then(function (browser) {
								if (browser) {
									return db('DeviceBrowsers').where({ DeviceID: device.ID, Name: device.BrowserName }).update({ Latest: 0 })
										.then(function () {
											return db('DeviceBrowsers').where('ID', browser.ID).update({
												ClientID: device.ClientID,
												PushToken: req.body.PushToken || browser.PushToken,
												OneSignalUserID: req.body.OneSignalUserID || browser.OneSignalUserID,
												Latest: 1
											})
										})
								}
