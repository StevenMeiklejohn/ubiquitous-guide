	...
	queue: function (opts) { //videoID, artworkID, priority, deactivate) {
		return new Promise(function (resolve, reject) {
			...
			if (!videoID && !artworkID) {
				...
				db('VuforiaTargets').update({ TrackingRating: null, SyncRequired: true, Priority: priority, Deactivate: deactivate }).where('ArtworkID', artworkID)
					 .then(function () {
	...
	processQueue: function () {
		return new Promise(function (resolve, reject) {
			...
			.then(function () {
				...
				.from('ActivCanvasQueue as acq')
				.where({ Transcoded: true, SentToVuforia: false, Deactivate: false })
				.andWhere('UploadAttempts', '<', config.activCanvas.maxUploadAttempts)
	...
	start: function() {
		if (!timeout) {
			...
			monitor = setInterval(function() {
				...
				var cutOff2 = new Date(new Date().setHours(new Date().getHours() - 24));
				db('ActivCanvasQueue').where('SentToVuforia', true).andWhere('updated_at', '<', cutOff2).del()
					.then(function(removed1) {

						var cutOff3 = new Date(new Date().setHours(new Date().getHours() - (24 * 7)));
						db('ActivCanvasQueue').where('updated_at', '<', cutOff3).del()
