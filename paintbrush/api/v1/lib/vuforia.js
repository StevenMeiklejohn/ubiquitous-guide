			...
			.then(function(artwork) {
				...
				Promise.settle(queue)
					.then(function(results) {
						...
						var body ='<p>A duplicate image was detected while trying to enable <strong>' + artwork.Name + '</strong> for ActivCanvas.</p>' +
							'<p>This artwork has been automatically deactivated to prevent a conflict with the existing target.</p>' +
						...
						results.forEach(function(result) {
							...
							body +=
								'<div class="list-item">' +
									'<span><span class="bullet green"></span></span>' +
									'<span class="text"><span class="label">Artwork Name:</span> ' + (_details.Name || 'Unknown') + '</span>' +
								'</div>' +
								'<div class="list-item">' +
									'<span><span class="bullet green"></span></span>' +
									'<span class="text"><span class="label">Artist Name:</span> ' + (_details.ProfileID ? '<a href="/profile/' + _details.ProfileID + '">' + _details.ProfileName + '</a>' : 'Unkown') + '</span>' +
								'</div>';
						});

						body += '</div>' +
							'<p><strong>Please contact us at <a href="mailto:hello@artretailnetwork.com">hello@artretailnetwork.com</a> if you are unsure how to resolve this issue.</strong></p>';
				...
				body:
					...
					'<div class="bullet-list">' +
						...
						'<div class="list-item">' +
							...
							'<span class="text rating"><span class="label">Tracking Rating:</span> ' +
...
var Vuforia = {
	...
	//
	getEncodedImage: function(uri, forceScale, adjustedContrast) {
		return new Promise(function (resolve, reject) {
					...
					res.on('data', function (chunk) {
						...
						if (!forceScale && !adjustedContrast && size < limit) {
							...
							var ext = urlData.path.split('.').pop().toLowerCase(),
								tempFile = config.proxy.cachePath + uuid.v4() + '.' + ext;


							fs.writeFile(tempFile, buffer, 'binary', function () {

								im.identify(tempFile, function (err, features) {
				...
				if (!artwork) {
					...
					.then(function (target) {
						...
						var name = (env === 'production' ? '' : env.toUpperCase().substr(0,3) + '_' )
								+ (artwork.ArtistName.substr(0, 20) + '_' + artwork.Name.substr(0, 25) + '_' + artworkID).replace(/ /g, '-');
					...
					.then(function (target) {
						...
						return Vuforia.generateMetadata(artworkID, videoID)
							.then(function (metadata) {
								...
								return Vuforia.getEncodedImage(artwork.ImageURI, target.BadImage, adjustedContrastLevel)
									.then(function (image) {
				...
				.then(function (target) {
					...
					return db('VuforiaTargets')
						...
						.then(function () {
							...
							if (target.tracking_rating !== null && (target.previous_tracking_rating !== target.tracking_rating)) {
								return db('Artworks').first('ArtistProfileID', 'OwnerProfileID').where({ ID: artworkID })
									.then(function(artwork) {
										Socket.emit('artwork/activation', { profileID: [artwork.ArtistProfileID, artwork.OwnerProfileID] }, {
											ArtworkID: artworkID,
											TrackingRating: target.tracking_rating,
											InitialTrackingRating: target.initial_tracking_rating,
