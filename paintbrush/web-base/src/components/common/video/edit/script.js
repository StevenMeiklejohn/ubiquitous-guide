...
.directive('videoEdit', [function () {
	return {
		templateUrl: 'components/common/video/edit/tpl.htm',
		restrict: 'E',
		controller: ['$scope', '$stateParams', '$element', '$location', 'videoService', function ($scope, $stateParams, $element, $location, videoService) {
			...
			var form,
				...
				isReady = function () {
					...
					if ($scope.activeProfile.ActivCanvas.StatusID!==3) {
			...
			if ($stateParams.videoID) {
				videoService.get($stateParams.videoID).then(function(data) {
					$scope.video = data;
					isReady();
				});
			}
			...
			$scope.videoUploadConfig = {
				...
				beforeUpload: function(file, start) {
					...
					video.onloadedmetadata = function () {
						...
						if (duration > limit && !($scope.activeProfile.Artist && $scope.activeProfile.Artist.Private)) {
			...
			$scope.save = function () {
				...
				if (uploadInProgress) {
					...
					data.ProfileID = $scope.activeProfile.ID;
					...
					if ($scope.video && $scope.video.ID) {
						...
						videoService.add(data, $scope.$root.activeProfile.ID).then(complete).catch(error);
					}
