...
function exec2(file, args /*, options, callback */) {
  ...
  var child;
  try {
    child = childproc.spawn(file, args);
  }
  catch (e) { };
  ...
  if (child) {
      ...
      var limitedWrite = function(stream) {
        return function(chunk) {
          ...
          if (!killed && stream.contents.length > options.maxBuffer) {
            console.log('\x1b[33mIMAGEMAGICK\x1b[0m: \x1b[31mEXCEDED MAX BUFFER\x1b[0m: ' + (stream.contents.length / 1024) + '/' + (options.maxBuffer / 1024) + 'mb');
            bufferExceeded = true;
    ...
    // re-nice child process to lowest possible priority
    try {
      var command = [ 'renice -n', options.priority, '-p', child.pid ].join(' ');
      childproc.exec(command, function(err, stderr, stdout) { });
    }
    catch(e) {}
...
exports.identify = function(pathOrArgs, callback) {
  ...
  var proc = exec2(exports.identify.path, args, {timeout:120000}, function(err, stdout, stderr) {
    ...
    if (!err) {
      if (isCustom) {
        ...
        geometry = (result['geometry']||'0x0').split(/x/);
  ...
  if (!proc) {
    callback('Failed to initialise child process');
  }
  return proc;
}
